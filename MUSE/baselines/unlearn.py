...
from baselines import it_unlearn, tv_unlearn, finetune, prism_unlearn
...
def get_args():
    ...
    parser.add_argument(
        '--sam_rho', type=float, default=0.01,
        help="for sam"
    )

    parser.add_argument(
        '--pretrained_probe_path', type=str, default=None,
        help="Path to the pretrained probe model. If provided, enables adversarial unlearning."
    )
    parser.add_argument(
        '--adv_gamma', type=float, default=0.005,
        help="Weight for the adversarial loss."
    )
    parser.add_argument(
        '--select_layer', type=int, default=32,
        help="The layer from which to extract hidden states for the probe."
    )
